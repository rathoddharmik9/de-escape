...
export default function EditEventForm({ event }: EditEventFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setErrors((prev) => ({ ...prev, submit: res.message || "Failed to update event." }));
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-[760px] mx-auto pb-12">
      {/* Header */}
      <div className="mb-8">
        <Link href={`/admin/events/${event.id}`} className="text-xs text-[var(--ink-mute)] hover:text-[var(--green-ink)] transition-colors">
          ← Back to event details
        </Link>
        <h1 className="font-display text-3xl text-[var(--green-ink)] mt-2 tracking-tight">Edit Event</h1>
      </div>
