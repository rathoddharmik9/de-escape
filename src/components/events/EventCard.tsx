...
export default function EventCard({ event, index = 0 }: Props) {
  ...
  return (
    <Link
      ...
      className="group block rounded-3xl overflow-hidden glass border border-[var(--glass-border)] transition-all duration-500 hover:-translate-y-1.5 hover:border-white/25 focus-visible:outline-[var(--coral)]"
    ...
    >
      ...
      <div className="aspect-[4/5] relative overflow-hidden">
        {event.cover_image_url ? (
          ...
          <div
            className="absolute inset-0 flex items-center justify-center font-serif text-[88px] text-white/90 transition-transform duration-700 group-hover:scale-105"
        ...
        {/* Status badge */}
        {soldOut && (
          <span className="absolute top-3.5 right-3.5 z-10 text-[10px] font-medium tracking-widest uppercase px-3 py-1.5 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-[var(--ink-2)]">
            Sold out
          </span>
        )}
      </div>
      ...
      {/* Body */}
      <div className="p-5">
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-[var(--peach)] mb-2">
        ...
        <p className="mt-1.5 text-[13px] text-[var(--ink-2)] flex items-center gap-1.5">
        ...
        <div className="mt-4 pt-4 border-t border-[var(--glass-border)] flex justify-between items-center">
          <span className="font-serif text-xl text-[var(--ink)]">
            ...
            {event.price_paise > 0 && (
              <small className="text-xs text-[var(--ink-3)] font-sans ml-1">/seat</small>
            )}
          </span>
          ...
          {!soldOut && (
            <span
              className={`text-[11px] uppercase tracking-wider flex items-center gap-1.5 ${
                almostFull ? "text-[var(--amber)]" : "text-[var(--ink-2)]"
              }`}
            >
              {almostFull && (
                <span
